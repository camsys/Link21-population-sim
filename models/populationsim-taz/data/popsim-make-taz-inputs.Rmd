...
taz_shape_filename <- paste0(interim_dir, "taz_boundary/Link21_TAZ_Merged_FINAL_v15.shp")
...
```{r make-geography}
renumber_join_df <- select(renumber_df, TAZ_old = LINK21_TAZ, TAZ_new = TAZ_Renum)
...
crosswalk_df <- taz_df %>%
  ...
  rename(TAZ_old = TAZ) %>%
  left_join(., renumber_join_df, by = c("TAZ_old")) %>%
  rename(TAZ = TAZ_new) %>%
  select(-TAZ_old) %>%
  relocate(TAZ)
