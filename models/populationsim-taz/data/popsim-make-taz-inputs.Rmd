---
title: "Populationsim -- Make TAZ Inputs"
output: html_notebook
---

Dollars
TAZ $2017
ADJINC to get $2018

County needs to nest in PUMA to work
Try replacing County with PUMA
- make PUMA level control
- add to settings

# Overhead
```{r overhead, include = FALSE}
packages_vector <- c("tidyverse",
                     "readxl",
                     "sf")

need_to_install <- packages_vector[!(packages_vector %in% installed.packages()[,"Package"])]

if (length(need_to_install)) install.packages(need_to_install)

for (package in packages_vector) {
  library(package, character.only = TRUE)
}

```

# Remote I-O
```{r remote-io}
interim_dir <- "./"
external_dir <- "./"
model_dir <- "./"

# excel_filename <- paste0(interim_dir, "PopSim Control Totals 03-25-2022.xlsx")
excel_filename <- paste0(interim_dir, "2. PopSim Control Totals 2015_revised.xlsx")
# taz_correspondence_excel_filename <- paste0(interim_dir, "TAZ Renumbers v15.xlsx")
puma_shape_filename <- paste0(external_dir, "census/tl_2010_06_puma10.shp")
taz_centroid_shape_filename <- paste0(interim_dir, "taz_boundary/Link21_TAZ_v26_Centroids.shp")

output_crosswalk_filename <- paste0(model_dir, "geo_crosswalk.csv")
output_taz_control_filename <- paste0(model_dir, "control_totals_taz.csv")
output_county_control_filename <- paste0(model_dir, "control_totals_county.csv")
output_countypuma_control_filename <- paste0(model_dir, "control_totals_countypuma.csv")
output_combined_county_control_filename <- paste0(model_dir, "control_totals_countycomb.csv")
output_sub_region_control_filename <- paste0(model_dir, "control_totals_subregion.csv")
output_region_control_filename <- paste0(model_dir, "control_totals_region.csv")
output_single_control_filename <- paste0(model_dir, "control_totals_single.csv")
```

# Parameters
```{r parameters}
LAT_LNG_EPSG <- 4326
PLANAR_EPSG <- 3857

worker_controls_df <- tibble(county_name = c("Alameda", 
                                             "Contra Costa",
                                             "Marin",
                                             "Napa",
                                             "San Francisco",
                                             "San Mateo",
                                             "Santa Clara",
                                             "Solano",
                                             "Sonoma"),
                             POPWRKR = c(827795,
                                         536209,
                                         130747,
                                         71146,
                                         505329,
                                         405183,
                                         967863,
                                         200569,
                                         250010))
                                                
```

# Data Reads
```{r data-reads}
taz_df <- read_excel(excel_filename, sheet = "TAZ")
county_df <- read_excel(excel_filename, sheet = "County", skip = 1L)
# region_df <- read_excel(excel_filename, sheet = "Region", skip = 1L)

# renumber_df <- read_excel(taz_correspondence_excel_filename, sheet = "Correspondence")

taz_centroid_sf <- st_read(taz_centroid_shape_filename)
puma_sf <- st_read(puma_shape_filename)
```

# Make Geometry Crosswalk
```{r make-geography}
taz_puma_df <- puma_sf %>%
  mutate(PUMA = as.integer(PUMACE10)) %>%
  st_transform(., crs = PLANAR_EPSG) %>%
  select(PUMA, geometry) %>%
  st_join(st_transform(taz_centroid_sf, crs = PLANAR_EPSG), ., join = st_intersects) %>%
  st_drop_geometry(.) %>%
  rename(TAZ = NEW_TAZ)

county_names_df <- select(taz_df, county_name = County) %>%
  distinct(.) %>%
  mutate(COUNTY = row_number())
  
county_puma_df <- taz_df %>%
  select(TAZ, county_name = County) %>%
  left_join(., county_names_df, by = c("county_name")) %>%
  left_join(., taz_puma_df, by = c("TAZ")) %>%
  distinct(COUNTY, PUMA) %>%
  arrange(COUNTY, PUMA) %>%
  mutate(COUNTYPUMA = row_number())
  
crosswalk_df <- taz_df %>%
  select(TAZ, county_name = County) %>%
  mutate(REGION = 1L) %>%
  left_join(., taz_puma_df, by = c("TAZ")) %>%
  left_join(., county_names_df, by = c("county_name")) %>%
  select(-county_name) %>%
  select(TAZ, COUNTY, PUMA, REGION) %>%
  left_join(., county_puma_df, by = c("COUNTY", "PUMA"))

check_df <- crosswalk_df %>%
  distinct(COUNTY, PUMA) %>%
  group_by(PUMA) %>%
  summarise(count = n(), .groups = "drop") %>%
  filter(count > 1)

check_cross_df <- filter(crosswalk_df, PUMA %in% check_df$PUMA)

write_csv(crosswalk_df, output_crosswalk_filename)

```

# Make TAZ Control
```{r taz-control}
taz_control_df <- taz_df %>%
  select(TAZ,
         HHLESSGQ = HH,
         HHCHILDNO = `No Children`,
         HHCHILDYES = Children,
         HHINC1 = `Less than 25k`,
         HHINC2 = `25-50k`,
         HHINC3 = `50-75k`,
         HHINC4 = `75-100k`,
         HHINC5 = `100-150k`,
         HHINC6 = `150k+`,
         HHGQYES = GQ,
         HHGQNO = `HH Pop (nonGQ)`) %>%
  mutate(HHBASE = HHLESSGQ + HHGQYES)

write_csv(taz_control_df, output_taz_control_filename)
```

# Make County Puma Control
```{r county-puma-control}
county_names_df <- select(taz_df, county_name = County) %>%
  distinct(.) %>%
  mutate(COUNTY = row_number())

join_taz_df <- taz_control_df %>%
  left_join(., select(crosswalk_df, TAZ, COUNTYPUMA, COUNTY), by = c("TAZ")) %>%
  select(-TAZ) %>%
  group_by(COUNTY, COUNTYPUMA) %>%
  summarise_all(sum)
  
county_puma_control_df <- county_df %>%
  filter(County != "Total") %>%
  select(county_name = County,
         HHSIZE1 = `1...4`,
         HHSIZE2 = `2...5`,
         HHSIZE3 = `3`,
         HHSIZE4 = `4+`,
         HHWORK0 = `0`,
         HHWORK1 = `1...9`,
         HHWORK2 = `2...10`,
         HHWORK3 = `3+`) %>%
  left_join(., county_names_df, by = c("county_name")) %>%
  left_join(join_taz_df, ., by = c("COUNTY")) %>%
  group_by(COUNTY) %>%
  mutate(share = HHBASE / sum(HHBASE)) %>%
  mutate(HHSIZE1 = round(HHSIZE1 * share),
         HHSIZE2 = round(HHSIZE2 * share),
         HHSIZE3 = round(HHSIZE3 * share),
         HHSIZE4 = round(HHSIZE4 * share),
         HHWORK0 = round(HHWORK0 * share),
         HHWORK1 = round(HHWORK1 * share),
         HHWORK2 = round(HHWORK2 * share),
         HHWORK3 = round(HHWORK3 * share)) %>%
  ungroup() %>%
  select(-share, -COUNTY, -county_name)

write_csv(county_puma_control_df, output_countypuma_control_filename)

```

# Make County Control
```{r county-control}
join_taz_df <- taz_control_df %>%
  left_join(., select(crosswalk_df, TAZ, COUNTY), by = c("TAZ")) %>%
  select(-TAZ) %>%
  group_by(COUNTY) %>%
  summarise_all(sum)

join_worker_control_df <- worker_controls_df %>%
  left_join(., county_names_df, by = c("county_name")) %>%
  select(-county_name)
  
county_control_df <- county_df %>%
  filter(County != "Total") %>%
  select(county_name = County,
         HHSIZE1 = `1...4`,
         HHSIZE2 = `2...5`,
         HHSIZE3 = `3`,
         HHSIZE4 = `4+`,
         HHWORK0 = `0`,
         HHWORK1 = `1...9`,
         HHWORK2 = `2...10`,
         HHWORK3 = `3+`,
         POPAGE1 = `0-4`,
         POPAGE2 = `5-17`,
         POPAGE3 = `18-24`,
         POPAGE4 = `25-34`,
         POPAGE5 = `35-64`,
         POPAGE6 = `65+`,
         POPSEX1 = Male,
         POPSEX2 = Female) %>%
  left_join(., county_names_df, by = c("county_name")) %>%
  left_join(., join_taz_df, by = c("COUNTY")) %>%
  select(-county_name) %>%
  arrange(COUNTY) %>%
  relocate(COUNTY) %>%
  left_join(., join_worker_control_df, by = c("COUNTY"))

write_csv(county_control_df, output_county_control_filename)
```


# Make Single Control for Testing
```{r single-control}
single_control_df <- county_control_df %>%
  mutate(SINGLE = 1L) %>%
  select(-COUNTY) %>%
  group_by(SINGLE) %>%
  summarise_all(sum) %>%
  mutate(PERSBASE = 7582788)

write_csv(single_control_df, output_single_control_filename)  
```

